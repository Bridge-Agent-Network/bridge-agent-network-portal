...
const networkUpdates = [
  { title: "Live Call in 2 hours", detail: "Winning the Renovation Conversation", icon: CalendarDays },
  { title: "New Resource Added", detail: "Seller Options Presentation v2", icon: FileText },
  { title: "3 New Opportunities", detail: "Review and take action", icon: Handshake },
  { title: "You earned 125 Bridge Points", detail: "Great work! Keep the momentum.", icon: CalendarClock }
];
...
const quickActions = [
  ...
  { label: "Update Profile", icon: UserCircle, href: "#directory" }
];
...
function CardHeader({ title, action }: { title: string; action?: string }) {
  return (
    <div className="mb-4 flex items-center justify-between gap-3">
      ...
      {action ? <a className="text-xs font-bold text-bridge-blue hover:text-bridge-navy" href="#">{action}</a> : null}
    </div>
  );
}
    ...
    <div id="dashboard" className="grid gap-4">
      <div className="grid gap-4 xl:grid-cols-[1.16fr_0.98fr_1.05fr]">
        <DashboardCard className="relative overflow-hidden p-5">
          ...
          <div className="relative divide-y divide-slate-100">
            {networkUpdates.map((item) => (
              <a key={item.title} href="#" className="flex items-center gap-4 py-3.5">
        ...
        <DashboardCard id="events" className="overflow-hidden p-5">
          <CardHeader title="Upcoming Live Call" action="View All" />
          <div className="relative min-h-[265px] overflow-hidden rounded-lg bg-[radial-gradient(circle_at_75%_25%,#164b7e,#062a4a_58%,#031a30)] p-6 text-white">
            ...
            <div className="absolute bottom-0 right-8 flex h-56 w-36 items-start justify-center rounded-t-full bg-gradient-to-b from-slate-200/95 to-slate-500/75 pt-8 text-5xl font-bold text-bridge-navy">
              RS
            </div>
        ...
        <DashboardCard id="points" className="p-5">
          <CardHeader title="Bridge Points" action="View Details" />
          <div className="grid gap-5 sm:grid-cols-[1fr_150px] sm:items-center">
            <div>
              ...
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-200">
                <div className="h-full w-[82%] rounded-full bg-bridge-gold" />
              </div>
            ...
            <div className="mx-auto flex h-36 w-36 items-center justify-center rounded-full border-[8px] border-slate-200">
              <div className="flex h-24 w-24 items-center justify-center rounded-full border-4 border-bridge-gold bg-[radial-gradient(circle,#62411a,#1f170e)] text-bridge-gold shadow-lg">
          ...
          <div className="mt-5 grid grid-cols-3 divide-x divide-slate-200 border-t border-slate-200 pt-5 text-center">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-emerald-600">350</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">This Quarter</p>
              <p className="mt-1 text-2xl font-bold text-emerald-600">1,125</p>
            </div>
            <div>
              <p className="text-sm text-slate-500">All Time</p>
              <p className="mt-1 text-2xl font-bold text-slate-950">2,450</p>
            </div>
          </div>
        </DashboardCard>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.16fr_0.98fr_1.05fr]">
        <DashboardCard id="resources" className="p-5">
          <CardHeader title="Resource Library" action="View All" />
            ...
            {["All Resources", "Presentations", "Tools & Calculators", "Guides", "Marketing"].map((tab, index) => (
              <button
                ...
                className={`rounded-md border px-4 py-2 text-xs font-bold ${
                  index === 0 ? "border-bridge-navy bg-bridge-navy text-white" : "border-slate-200 bg-white text-slate-600 hover:border-bridge-blue"
                }`}
              >
                {tab}
              </button>
          ...
          <div className="divide-y divide-slate-100">
            {resourceRows.map((resource) => (
              <div key={resource.title} className="grid grid-cols-[64px_1fr_auto_auto] items-center gap-4 py-3">
                <div className={`flex h-12 w-16 items-center justify-center rounded ${resource.color} text-white`}>
                  <resource.icon className="h-5 w-5" />
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-bold text-slate-950">{resource.title}</p>
                  <p className="truncate text-xs text-slate-500">{resource.meta}</p>
                </div>
                <button className="hidden rounded-md border border-slate-200 px-4 py-2 text-xs font-bold text-bridge-navy hover:border-bridge-blue sm:inline-flex">
                  Download
                </button>
          ...
          <a href="#resources" className="mt-3 flex items-center justify-center gap-2 border-t border-slate-100 pt-4 text-sm font-bold text-bridge-blue">
            View All Resources
        ...
        <DashboardCard id="leaderboard" className="p-5">
          <CardHeader title="Top Agents This Month" action="View Leaderboard" />
          <div className="divide-y divide-slate-100">
            {leaderboard.map((agent, index) => (
              <div key={agent.name} className={`grid grid-cols-[32px_42px_1fr_auto] items-center gap-3 rounded-md px-3 py-3 ${agent.active ? "bg-slate-100" : ""}`}>
                ...
                <div className="min-w-0">
                  <p className={`truncate text-sm font-bold ${agent.active ? "text-bridge-blue" : "text-slate-950"}`}>{agent.name}</p>
                  <p className="truncate text-xs text-slate-500">{agent.territory}</p>
                </div>
                <p className={`text-sm font-bold ${index < 3 ? "text-bridge-gold" : "text-bridge-blue"}`}>{agent.points.toLocaleString()}</p>
              </div>
            ))}
          </div>
          <a href="#leaderboard" className="mt-3 flex items-center justify-center gap-2 border-t border-slate-100 pt-4 text-sm font-bold text-bridge-blue">
        ...
        <div className="grid gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {quickActions.map((action) => (
                <a
                  key={action.label}
                  href={action.href}
                  className="group flex min-h-[88px] flex-col items-center justify-center rounded-md border border-slate-200 bg-white p-3 text-center text-sm font-bold text-bridge-navy transition hover:border-bridge-blue hover:bg-bridge-sky/40"
                >
          ...
          <DashboardCard className="p-5">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-sm font-extrabold uppercase tracking-wide text-bridge-navy">Your Impact</h2>
      ...
      <div className="sr-only">
        <span id="opportunities">Opportunities</span>
        <span id="marketing">Marketing Center</span>
        <span id="training">Training Center</span>
        <span id="books">Book Requests</span>
